...
public class ModelosDemoPage extends BasePage {
        ...
        public ModelosDemoPage() {
                ...
                // ============================================================
                // EXEMPLOS PRÁTICOS: LOADABLEDETACHABLEMODEL
                // ============================================================
                add(new Label("exemploLoadableDetachableTitulo", "Exemplos Práticos: LoadableDetachableModel"));

                // Exemplo 1: Criar LoadableDetachableModel com ID
                // O objeto será carregado do repositório apenas quando necessário
                CertidaoLoadableDetachableModel loadableModel = new CertidaoLoadableDetachableModel(1L);

                // Usar PropertyModel para acessar propriedades do LoadableDetachableModel
                add(new Label("exemploLoadableNumero",
                                new PropertyModel<>(loadableModel, "numero")));
                add(new Label("exemploLoadableInteressado",
                                new PropertyModel<>(loadableModel, "interessado")));
                add(new Label("exemploLoadableTipo",
                                new PropertyModel<>(loadableModel, "tipo")));
                add(new Label("exemploLoadableDataEmissao",
                                new PropertyModel<>(loadableModel, "dataEmissao")));
                add(new Label("exemploLoadableStatus",
                                new PropertyModel<>(loadableModel, "status")));

                // Exemplo 2: Comparação Model.of() vs LoadableDetachableModel
                add(new Label("comparacaoLoadableTitulo", "Comparação: Model.of() vs LoadableDetachableModel"));

                // Model.of() - mantém objeto em memória
                IModel<Certidao> modelOf = Model.of(certidaoExemplo);
                add(new Label("comparacaoModelOfNumero",
                                new PropertyModel<>(modelOf, "numero")));

                // LoadableDetachableModel - carrega sob demanda
                // Usar ID 1L que existe no repositório (não usar certidaoExemplo.getId() que é
                // 999L)
                CertidaoLoadableDetachableModel loadableModelComparacao = new CertidaoLoadableDetachableModel(1L);
                add(new Label("comparacaoLoadableNumero",
                                new PropertyModel<>(loadableModelComparacao, "numero")));
        }
